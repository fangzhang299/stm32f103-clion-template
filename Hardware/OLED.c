 ...
 * @param  FraLength 小数部分长度，建议范围：0~6
 * @retval 无
 */
void OLED_ShowFloatNum(uint8_t Line, uint8_t Column, float Number, uint8_t IntLength, uint8_t FraLength)
{
	...
	Scale = OLED_Pow(10, FraLength);
	IntegerPart = (uint32_t)Number;
	FractionPart = (uint32_t)((Number - IntegerPart) * Scale + 0.5f);
	...
	OLED_ShowNum(Line, Column + 1, IntegerPart, IntLength);
	if (FraLength > 0)
	{
		OLED_ShowChar(Line, Column + IntLength + 1, '.');
		OLED_ShowNum(Line, Column + IntLength + 2, FractionPart, FraLength);
	}
}
